...
- `comment`: Post a comment with a cost estimate on GitHub/GitLab/Azure Repos pull request/commit
...
The Infracost CLI can post cost estimates as pull request/commit comments on GitHub, GitLab and Azure DevOps Repos. This is most useful when Infracost CLI runs in CI/CD pipeline.

Please 👍 [this issue](https://github.com/infracost/infracost/issues/1173) for Bitbucket support.

The command uses Infracost JSON files generated by `infracost breakdown --format json` command. Use `github`, `gitlab` or `azure-repos` subcommands to specify the CI platform. Run them with `--help` to see usage details.

You can choose the commenting behavior with the `--behavior` flag, `update` is a good default:

- `update`: Create a single comment and update it. The "quietest" option.
- `hide-and-new`: Minimize previous comments and create a new one. Supported only by GitHub comments.
...
- `--path`: required, accepts a string as a file path, JSON array or a glob pattern (needs quotes).
- `--repo`: required, use `$GITHUB_REPOSITORY` predefined environment variable.
- `--pull-request`: required to post on a pull request, can be extracted from workflows's event and set as an environment variable: `PR_NUMBER: ${{ github.event.number }}`. Mutually exclusive with `--commit` flag.
...
For GitHub Enterprise you can specify `--github-api-url` with `$GITHUB_API_URL`.
...
- `--path`: required, accepts a string as a file path, JSON array or a glob pattern (needs quotes).
- `--repo`: required, use `$CI_PROJECT_PATH` predefined environment variable.
...
For GitLab for Enterprise you can specify `--gitlab-server-url` with `$CI_SERVER_URL`.
...
- `--path`: required, accepts a string as a file path, JSON array or a glob pattern (needs quotes).
- `--repo`: required, use `$BUILD_REPOSITORY_NAME` predefined environment variable.
