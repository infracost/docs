...
The [Infracost GitHub App](https://github.com/marketplace/infracost) is verified by GitHub, and installed in thousands of GitHub orgs. The App needs _write access to pull requests_ so it can post a comment with any cost estimates, tagging, and FinOps policy issues. You can select the repos you would like to give access to the App.
...
- **Read-only** access is needed for the following so Infracost gets notified of repo or pull request changes and can process the code:
  ...
  - Merge queues
  - Copilot chat and editor context
...
The "Infracost (Limited)" GitHub App requires the following permissions. Since this App only has Read-only on "Contents", it does not work with the [AutoFix pull requests](https://www.infracost.io/blog/ai-for-finops-fix-cloud-cost-issues-faster/) feature.
