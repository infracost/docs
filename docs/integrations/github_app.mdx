...
Note that when engineers [dismiss or snooze](#dismiss-or-snooze-policy-issues) pull requests, Infracost returns a "passed" status check to GitHub and unblocks the pull requests. Infracost automatically returns the "skipped" status check when a repo does not have any Terraform.
