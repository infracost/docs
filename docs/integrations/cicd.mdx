...
export const Grid = ({ logos }) => {
  ...
  return <div className="ci-grid">{items}</div>;
};

export const CIBox = ({ image, link, maxHeight, newWindow, text }) => (
  <a className="ci-box" href={link}>
    <img
      src={image}
...
<img
  src={useBaseUrl('img/screenshots/actions-pull-request.png')}
  alt="Infracost pull request comment"
/>
...
<Grid
  logos={[
    ...
    },
    {
      ...
      link: 'https://github.com/infracost/infracost-azure-devops',
      newWindow: true,
    },
    {
...
To test the bleeding edge version (not recommended for production deployments), use the [`infracost/infracost:ci-edge`](https://hub.docker.com/r/infracost/infracost/tags) image or the [preview build](https://github.com/infracost/infracost/releases/tag/preview).
