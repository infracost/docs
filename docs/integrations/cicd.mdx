...
The above CI/CD integrations [parse Terraform HCL code](/docs/features/cli_commands/#option-1-terraform-directory) directly (with the exception of Atlantis). That is our recommended way of running Infracost since it's faster and does not require cloud credentials to generate a Terraform plan JSON. However, Infracost can also be used alongside deployment systems that already generate a Terraform plan JSON file, for example:
