...
We kicked-off 2023 with a focus on helping companies save money on their cloud bills by fitting into their engineering CI/CD workflows.

Last month, we started tracking cost savings from Guardrails. We also made Infracost run faster by only running on changed directories, caching modules, and running projects in parallel. On top of that, we made numerous bug fixes to make Terragrunt support more robust, and released support for single sign-on.
...
Engineers love fast CI/CD builds so in January we made three major changes to make Infracost run even faster.
...
Hassan, our CEO, recorded a [new YouTube demo](https://www.youtube.com/watch?v=DDi6GE9RIik) showing how Infracost Cloud builds on top of Infracost open source and gives team leads, managers and FinOps practitioners dashboards, guardrails and centralized cost policies so they can save money (e.g. switch AWS GP2 volumes to GP3).
...
- [JeongSuk Lee](https://www.inflearn.com/users/@jeongsuk): [[DevOps 정석 3] GitHub Actions으로 제대로 구축하는 CI 대시보드](https://www.inflearn.com/course/%EB%8D%B0%EB%B8%8C%EC%98%B5%EC%8A%A4-%EC%A0%95%EC%84%9D-%EA%B9%83%ED%97%99%EC%95%A1%EC%85%98-ci#curriculum) (We haven't done this course ourselves as we don't speak Korean)
